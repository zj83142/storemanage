...
public class NfcToolsBack {
	...
	public void readInfo(Intent intent, Tag mytag, TextView infoTv,
			boolean isWrite, Context context, Handler handler) {
				...
				if (isWrite) {
					...
					if (judgeNfcInfo(result)) {
						...
						if (write(info, myTag)) {
							if (infoTv != null) {
								infoTv.setText(info);
								handler.sendEmptyMessage(ParamsUtil.QUERY_NFC);
							}
						} else {
							showToast("读入错误！");
						}
